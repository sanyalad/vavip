...
export const useVideoStack = ({
  ...
  // Initialize section positions and ensure backdrop is hidden
  useEffect(() => {
    // Ensure backdrop is hidden on mount
  ...
  const snapToSection = useCallback((targetIndex: number, _fromPercent: number, dir: 'up' | 'down') => {
  ...
  // snapBack function reserved for future use if needed
    ...
    if (footer && backdrop) {
      gsap.timeline({
        onComplete: () => {
          ...
          setIsAnimating(false);
          // Ensure backdrop is fully hidden
