...
const VideoSection = forwardRef<HTMLElement, VideoSectionProps>(function VideoSection(
  ...
  const primeVideo = useCallback(() => {
    ...
    video.preload = 'auto'
    // Force browser to fetch & decode a bit so ╨┐╨╡╤А╨▓╤Л╨╣ ╨┐╨╛╨║╨░╨╖ ╨╜╨╡ ╨╝╨╛╤А╨│╨░╨╡╤В
    try {
      video.load()
  ...
  // Caption animation - immediate response, no delay for active sections
  useEffect(() => {
    if (isActive) {
      // Show caption immediately when section becomes active
  ...
  // Prime when ╤Б╨╡╨║╤Ж╨╕╤П ╨┐╨╛╤П╨▓╨╗╤П╨╡╤В╤Б╤П ╨▓ viewport (╨┤╨░╨╢╨╡ ╨╡╤Б╨╗╨╕ ╨╜╨╡ ╨░╨║╤В╨╕╨▓╨╜╨░)
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {posterSrc && <img src={posterSrc} alt={`╨д╨╛╨╜: ${title}`} className={styles.videoPoster} />}
      </video>
      ...
      <h2 
        ...
        style={{
          ...
          transition: showCaption 
            ? 'opacity 0.4s ease-out, transform 0.35s cubic-bezier(0.16, 1, 0.3, 1)' 
